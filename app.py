...
def XGB_train_metrics(df, params_set):
    ...
    model_xgb = XGBClassifier(
        max_depth=params_set[0],
        eta=params_set[1],
        min_child_weight=params_set[2],
        subsample=params_set[3],
        colsample_bylevel=params_set[4],
        colsample_bytree=params_set[5],
    )
    # model = XGBClassifier()
...
def decision_boundary(df):
    ...
    plot_decision_regions(
        df_train_pca.values, y_train.values, clf=model, res=0.02, zoom_factor=5
    )
    st.pyplot()
...
def home_page_builder(df, data, rows, columns):
    ...
    if st.checkbox("Show Visualization"):
        fig = px.histogram(
            df.status, x="status", title='Distribution of Target Variable "status"'
        )
        st.plotly_chart(fig)
        st.write(
            "We can see Approved is about three times of Decliened, which may bring an imbalanced issue for prediction - we will deal with this issue during modeling."
        )
        st.write("-" * 60)
    ...
    if st.checkbox("Show Feature Summary"):
        ...
        st.write(
            "For each columns in our original dataset, we can see the statistics summary (Null Value Count, Unique Value Count, Data Type, etc.)"
        )
...
def logistic_page_builder(data):
    ...
    st.table(
        pd.DataFrame(
            data=[
                round(accuracy_reg * 100.0, 2),
                round(precision_reg * 100.0, 2),
                round(recall_reg * 100, 2),
                round(roc_auc_reg * 100, 2),
                round(f1_reg * 100, 2),
            ],
            index=[
                "Accuracy",
                "Precision (% we predicted as Declined are truly Declined)",
                "Recall (% Declined have been identified)",
                "ROC_AUC",
                "F1",
            ],
            columns=["%"],
...
def xgb_page_builder(data):
    ...
    (
        ...
        precision_xgb,
        model_xgb,
    ) = XGB_train_metrics(data, params_set)
    st.subheader("Model Introduction")
    st.write("")
    st.write(
        "XGBoost - e**X**treme **G**radient **B**oosting, is an implementation of gradient boosted **decision trees** designed for speed and performance, which has recently been dominating applied machine learning. We recommend you choose this model to do the prediction."
    )
    st.write(
        "See more from this blog: https://machinelearningmastery.com/gentle-introduction-xgboost-applied-machine-learning/"
    )
    st.write("")
    ...
    st.write("")
    st.markdown(
        "We separated the dataset to training and testing dataset, using training data to train our model then do the prediction on testing dataset, here's XGB prediction performance: "
    )
    st.write("")
    st.write(f"Running time: {(datetime.datetime.now() - start_time).seconds} s")
    st.table(
        pd.DataFrame(
            data=[
                round(accuracy_xgb * 100.0, 2),
                round(precision_xgb * 100.0, 2),
                round(recall_xgb * 100, 2),
                round(roc_auc_xgb * 100, 2),
                round(f1_xgb * 100, 2),
            ],
            index=[
                "Accuracy",
    ...
    fig = px.bar(
        feature_importance, x=feature_importance.columns, y=feature_importance.T
    )
...
def main():
    ...
    choose_model = st.sidebar.selectbox(
        "Choose the page or model", ["Home", "Logistic Regression", "XGB"]
    )
