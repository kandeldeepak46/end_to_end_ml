...
def drop_useless_columns(df, manual_drop_list=None):
    ...
    for col in df.columns:
        if df[col].nonunique() <= 1 or df[col] >= df.shape[0] * 0.95:
...
def data_preprocessing(df, manual_drop_list=None):
    """Returns processed data and columns list that need to be dropped"""
...
def feature_summary(data):
    ...
    logger.info("rows:", data.shape[0])
    logger.info("columns:", data.shape[1])
